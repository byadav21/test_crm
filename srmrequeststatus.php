...
$query = "SELECT sb.approve_status, sb.name as old_program_name, sb.batch_code as old_batch_code, ii.name as old_institute_name, bb.name as new_program_name, bb.batch_code as new_batch_code, s.name as student_name, s.email, s.mobile, tb.status, sb.bt_srm_comments,sb.bt_approver_comments, sb.bt_fee_waiver,sb.bt_srm_attachment, SUM(sp.amount) AS total from te_student_batch sb, 
	te_student s,te_transfer_batch tb, te_ba_batch bb, te_in_institutes ii, te_student_payment sp, te_student_te_student_batch_1_c tss where sb.id='".$student_batch."' and tss.te_student_te_student_batch_1te_student_batch_idb=sb.id
	and tss.te_student_te_student_batch_1te_student_ida=s.id and tb.batch_id_rel=sb.id and bb.id=tb.te_ba_batch_id_c and ii.id=sb.te_in_institutes_id_c and sp.te_student_batch_id_c=sb.id";
