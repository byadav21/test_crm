    ...
    else
    {
        ...
        */
        ...
        if ($lead_source != '' && $LBean->abnd_reenquired_status != 1 && in_array($LBean->lead_source, $ABNDArr))
        {
            ...
            createLog('{Entry into DIRECT condtions}', 'direct_captured_payment_' . date('Y-m-d') . '_log.txt', 'new lead_sourceX=='.$lead_sourceX.' | default='.$LBean->lead_source, $data);
        }
        else
        {
            $lead_sourceX = $LBean->lead_source;
            createLog('{Default old value reamin update}', 'direct_captured_payment_' . date('Y-m-d') . '_log.txt', '$lead_sourceX=='.$lead_sourceX, $data);
        }
