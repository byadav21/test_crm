...
$sql_program    = "SELECT Ts.name AS institute,
                 ...
                 WHERE Sb.id='" . $student_batch_id_old . "'";
...
$oldBatchSql     = "SELECT *
                    FROM te_student_batch
                    WHERE id='" . $student_batch_id_old . "'
                      AND deleted=0";
...
if (isset($_REQUEST['request_status']) && $_REQUEST['request_status'] == "Rejected")
{
    $GLOBALS['db']->query("UPDATE te_transfer_batch
                                            SET status='" . $_REQUEST['request_status'] . "',
                                         is_new_approved=1,
...
$srm_auto_ass_useridSql = "SELECT *
                                            FROM te_srm_auto_assignment
                                            WHERE te_ba_batch_id_c='" . $new_batch_id . "'
                                          AND deleted=0";
...
$batchSql               = "SELECT   b.id AS batch_id,
                                                    ...
                                                    b.fees_inr,
                                                    b.fees_in_usd,
                                                    b.initial_payment_inr,
                                                    b.initial_payment_usd,
                                                    b.initial_payment_date,
                                                    p.id AS program_id,
                                                    i.id AS institute_id,
                                                    b.total_sessions_planned,
                                                    b.batch_start_date,
                                                    b.minimum_attendance_criteria
                                             FROM te_ba_batch b
                                             INNER JOIN te_pr_programs_te_ba_batch_1_c pbr ON pbr.te_pr_programs_te_ba_batch_1te_ba_batch_idb=b.id
                                             INNER JOIN te_pr_programs p ON pbr.te_pr_programs_te_ba_batch_1te_pr_programs_ida=p.id
                                             INNER JOIN te_in_institutes_te_ba_batch_1_c bir ON b.id=bir.te_in_institutes_te_ba_batch_1te_ba_batch_idb
                                             INNER JOIN te_in_institutes i ON bir.te_in_institutes_te_ba_batch_1te_in_institutes_ida=i.id
                                             WHERE b.deleted=0
...
$studentPaymentSql = "SELECT SUM(te_student_payment.amount) AS total
                            FROM te_student_payment,
                                 te_student_te_student_payment_1_c
                            WHERE te_student_payment.id = te_student_te_student_payment_1_c.te_student_te_student_payment_1te_student_payment_idb
                              AND te_student_payment.te_student_batch_id_c='" . $student_batch_id_old . "'
                              AND te_student_payment.payment_realized=1
...
$GLOBALS['db']->query("UPDATE te_payment_details,
                                leads_te_payment_details_1_c
                         SET te_payment_details.deleted = 1,
                             leads_te_payment_details_1_c.deleted=1
                         WHERE te_payment_details.id = leads_te_payment_details_1_c.leads_te_payment_details_1te_payment_details_idb
                           AND leads_te_payment_details_1_c.leads_te_payment_details_1leads_ida='" . $oldBatchDetails['leads_id'] . "'");

#update leads
$GLOBALS['db']->query("UPDATE leads,
                            leads_cstm
                     ...
                     WHERE leads_cstm.id_c=leads.id
...
$insertRelSql = "INSERT INTO te_student_te_student_payment_1_c
                    SET id='" . create_guid() . "',
                date_modified='" . date('Y-m-d H:i:s') . "',
                deleted=0,
...
WHERE te_student_batch.id = te_student_te_student_batch_1_c.te_student_te_student_batch_1te_student_batch_idb
  ...
  AND te_student_batch.id='" . $student_batch_id_old . "'");

#unlink old student batch plan from student batch
$GLOBALS['db']->query("UPDATE te_student_payment_plan,
       te_student_batch_te_student_payment_plan_1_c
SET te_student_payment_plan.deleted = 1,
    te_student_batch_te_student_payment_plan_1_c.deleted=1
WHERE te_student_payment_plan.id = te_student_batch_te_student_payment_plan_1_c.te_student9d1ant_plan_idb
  AND te_student_batch_te_student_payment_plan_1_c.te_student_batch_te_student_payment_plan_1te_student_batch_ida='" . $student_batch_id_old . "'
  AND te_student_payment_plan.te_student_id_c='" . $student_id . "'");

#unlink old student from student payment for old batch payment
$GLOBALS['db']->query("UPDATE te_student_payment,
       te_student_te_student_payment_1_c
SET te_student_payment.deleted = 1,
    te_student_te_student_payment_1_c.deleted=1
WHERE te_student_payment.id = te_student_te_student_payment_1_c.te_student_te_student_payment_1te_student_payment_idb
  AND te_student_payment.te_student_batch_id_c='" . $student_batch_id_old . "'
  AND te_student_te_student_payment_1_c.te_student_te_student_payment_1te_student_ida='" . $student_id . "'");

#update currency
$GLOBALS['db']->query("UPDATE te_student_payment_plan,
       te_student_batch_te_student_payment_plan_1_c
SET te_student_payment_plan.currency ='" . $currency . "'
WHERE te_student_payment_plan.id = te_student_batch_te_student_payment_plan_1_c.te_student9d1ant_plan_idb
...
function updateStudentPaymentPlan($batch_id, $student_id, $amount, $student_country)
{
    ...
    if ($student_country == "" || strtolower($student_country) == "india")
    {
        ...
        while ($row            = $GLOBALS['db']->fetchByAssoc($paymentPlanObj))
        {
            ...
            {
                $GLOBALS['db']->Query("UPDATE te_student_payment_plan
                                        SET paid_amount_inr=paid_amount_inr+" . $restAmt . ",
                                            paid='Yes'
                                        WHERE id='" . $row['id'] . "'");
            ...
            {
                $GLOBALS['db']->Query("UPDATE te_student_payment_plan
                                        SET paid_amount_inr=paid_amount_inr+" . $amount . "
                                        WHERE id='" . $row['id'] . "'");
                $amount = 0;
            }
            #update balanced amount
            $GLOBALS['db']->Query("UPDATE te_student_payment_plan
                                    SET balance_inr=due_amount_inr-paid_amount_inr
                                    WHERE id='" . $row['id'] . "'");
            if ($amount == 0)
                break;
        }
    }else
    {
        ...
        while ($row            = $GLOBALS['db']->fetchByAssoc($paymentPlanObj))
        {
            ...
            {
                $GLOBALS['db']->Query("UPDATE te_student_payment_plan
                                        SET paid_amount_usd=paid_amount_usd+" . $restAmt . ",
                                            paid='Yes'
                                        WHERE id='" . $row['id'] . "'");
                $amount = $amount - $restAmt;
            }
            ...
            {
                $GLOBALS['db']->Query("UPDATE te_student_payment_plan
                                        SET paid_amount_usd=paid_amount_usd+" . $amount . "
                                        WHERE id='" . $row['id'] . "'");
                $amount = 0;
            }
            #update balanced amount
            $GLOBALS['db']->Query("UPDATE te_student_payment_plan
                                        SET balance_usd=due_amount_usd-paid_amount_usd
                                        WHERE id='" . $row['id'] . "'");
            if ($amount == 0)
                break;
        }
