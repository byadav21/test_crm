    ...
    case 'fetch_student':
      ...
      // print_r($batches);
        if($batches && count($batches)>0){
			...
			echo json_encode(array('result'=>array('id'=>$data['sid'],'name'=>$data['name'],'email'=>$data['email'],'status'=>$data['status']),'programme'=>$programm,'selbatch'=>$selBatch,'batch'=>array('id'=>$batches['id'],'name'=>$batches['name'],'id_org'=>$batches['batch_id'],'is_transfer'=>$isTransfer,'batch_code'=>$batches['batch_code'],'institute_name'=>$batches['institute_name']),'currentbatchid'=>$batches['id']));exit;
		}else{
