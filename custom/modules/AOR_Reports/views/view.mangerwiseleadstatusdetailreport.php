...
class AOR_ReportsViewMangerwiseleadstatusdetailreport extends SugarView
{
    ...
    public function display()
    {
		...
		$StatusList['payment_enquiry']     = 'Payment enquiry';
		$StatusList['program_enquiry']     = 'Program enquiry';
		$StatusList['miscellaneous']     = 'Miscellaneous';
		$StatusList['na']     = 'NA';
        ...
        if (isset($_POST['export']) && $_POST['export'] == "Export")
        {
            ...
            $file     = "Manager_wise_lead_statusdetailReport";
            $where    = '';
            $filename = $file . "_" . $from_date . "_" . $to_date;

            $leadSql = "SELECT count(l.id)total,lc.te_ba_batch_id_c,l.dristi_campagain_id,l.status_description FROM `leads` AS l inner join leads_cstm AS lc on l.id=lc.id_c WHERE l.deleted=0 $wherecl GROUP BY lc.te_ba_batch_id_c,l.`dristi_campagain_id`,l.status_description";
			...
			while ($row = $db->fetchByAssoc($leadObj))
			{
				...
				$programList[$batch_id .'_ManagerID_'.$manager_id][strtolower(str_replace(array(' ','-'),'_',$row['status_description']))][]   = $row['total'];
        ...
        $sugarSmarty->assign("StausListData", $StatusList);
        ...
        $sugarSmarty->display('custom/modules/AOR_Reports/tpls/mangerwiseleadstatusdetailreport.tpl');
    }
