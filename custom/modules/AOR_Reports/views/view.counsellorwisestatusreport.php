...
class AOR_ReportsViewCounsellorwisestatusreport extends SugarView
{
    ...
    public function display()
    {
        ...
        if (isset($_POST['export']) && $_POST['export'] == "Export")
        {
            ...
            $leadSql = "SELECT COUNT(l.id) AS lead_count,
                ...
                LEFT JOIN te_ba_batch ON lc.te_ba_batch_id_c = te_ba_batch.id
                 WHERE l.deleted=0
                   $wherecl
              GROUP BY l.status,l.assigned_user_id,te_ba_batch.batch_code order by  te_ba_batch.batch_code ";
            //echo $leadSql;exit();
            ...
            while ($row = $db->fetchByAssoc($leadObj))
			{
				...
				$programList[$row['assigned_user_id'] .'_BATCH_'.$row['batch_id']][strtolower($row['status'])]   = $row['lead_count'];
        ...
        $sugarSmarty->display('custom/modules/AOR_Reports/tpls/counsellorwisestatusreport.tpl');
    }
