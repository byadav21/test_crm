			...
			foreach ($res as $val){
				...
				if($val['total']>1){
					...
					if($res_insert_leads==1){
						...
						$update_leads_mark_duplicate = "UPDATE leads SET status='Duplicate',status_description='Duplicate',`duplicate_check`=1 WHERE id IN($leads_id_for_update)";
