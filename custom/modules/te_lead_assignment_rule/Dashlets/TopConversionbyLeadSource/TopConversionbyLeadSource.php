...
class TopConversionbyLeadSource extends Dashlet{ 
    ...
    protected function getCurrentBenchOutput()
    {
		...
		$leadQuery = "SELECT t1.name,COUNT(l.id) total_leads, GROUP_CONCAT(DISTINCT(l.utm)) AS lead_utm, COUNT(t7.id) converted_total FROM te_vendor t1 INNER JOIN te_vendor_te_utm_1_c t2 ON t1.id = t2.te_vendor_te_utm_1te_vendor_ida INNER JOIN te_utm t4 ON t4.id = t2.te_vendor_te_utm_1te_utm_idb INNER JOIN leads l ON l.utm = t4.name LEFT JOIN leads t7 ON l.id = t7.id AND t7.status='Converted' WHERE t4.utm_status = 'Live' AND l.deleted = 0 GROUP BY t1.name ORDER BY converted_total DESC LIMIT 0 ,".$this->top_leads;
