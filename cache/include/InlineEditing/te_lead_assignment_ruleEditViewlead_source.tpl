...
{if !isset($config.enable_autocomplete) || $config.enable_autocomplete==false}
	...
	<input type="hidden" id="{$fields.lead_source.name}_multiselect"
	name="{$fields.lead_source.name}_multiselect" value="true">
	{multienum_to_array string=$fields.lead_source.value default=$fields.lead_source.default assign="values"}
	<select id="{$fields.lead_source.name}"
	name="{$fields.lead_source.name}[]"
	multiple="true" size='6' style="width:150" title='' tabindex="1"  
     	>
	{html_options options=$fields.lead_source.options selected=$values}
	</select>
...
{else}
	...
	{assign var="ac_key" value=$smarty.capture.ac_key}
		...
		name="{$fields.lead_source.name}_multiselect" value="true">
		{multienum_to_array string=$fields.lead_source.value default=$fields.lead_source.default assign="values"}
		<select style='display:none' id="{$fields.lead_source.name}"
		name="{$fields.lead_source.name}[]"
		multiple="true" size='6' style="width:150" title='' tabindex="1"  
		        >
		{html_options options=$fields.lead_source.options selected=$values}
	...
	<span class="id-ff multiple">
	    <button type="button">
	    	<img src="{sugar_getimagepath file="id-ff-down.png"}" id="{$fields.lead_source.name}-image">
	    	</button>
	    	<button type="button"
	        id="btn-clear-{$fields.lead_source.name}-input"
	        title="Clear"
	        onclick="SUGAR.clearRelateField(this.form, '{$fields.lead_source.name}-input', '{$fields.lead_source.name};');SUGAR.AutoComplete.{$ac_key}.inputNode.updateHidden()"><img src="{sugar_getimagepath file="id-ff-clear.png"}"></button>
	</span>
	...
	YUI().use("autocomplete", "autocomplete-filters", "autocomplete-highlighters","node-event-simulate", function (Y) {
		...
		if({/literal}SUGAR.AutoComplete.{$ac_key}.minQLen{literal} == 0){
		    ...
		    function updateTextOnInterval(){
		    	if (document.activeElement != document.getElementById("{/literal}{$fields.lead_source.name}-input{literal}"))
