...
{if !empty($fields.te_vendor_id_c.value)}
{capture assign="detail_url"}index.php?module=te_vendor&action=DetailView&record={$fields.te_vendor_id_c.value}{/capture}
<a href="{sugar_ajax_url url=$detail_url}">{/if}
<span id="te_vendor_id_c" class="sugar_field" data-id-value="{$fields.te_vendor_id_c.value}">{$fields.source.value}</span>
{if !empty($fields.te_vendor_id_c.value)}</a>{/if}
{/if}
<div class="inlineEditIcon"> {sugar_getimage name="inline_edit_icon.svg" attr='border="0" ' alt="$alt_edit"}</div>			</td>
</tr>
{/capture}
