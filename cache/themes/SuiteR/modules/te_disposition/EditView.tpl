...
{capture name="label" assign="label"}{sugar_translate label='LBL_NAME' module='te_disposition'}{/capture}
...
{capture name="label" assign="label"}{sugar_translate label='LBL_ASSIGNED_TO_NAME' module='te_disposition'}{/capture}
{$label|strip_semicolon}:
</td>
{counter name="fieldsUsed"}
...
{capture name="label" assign="label"}{sugar_translate label='LBL_STATUS' module='te_disposition'}{/capture}
{$label|strip_semicolon}:
<span class="required">*</span>
</td>
{counter name="fieldsUsed"}
...
{capture name="label" assign="label"}{sugar_translate label='LBL_STATUS_DETAIL' module='te_disposition'}{/capture}
{$label|strip_semicolon}:
</td>
{counter name="fieldsUsed"}
...
{if !isset($config.enable_autocomplete) || $config.enable_autocomplete==false}
<select name="{$fields.status_detail.name}" 
id="{$fields.status_detail.name}" 
title=''       
>
{if isset($fields.status_detail.value) && $fields.status_detail.value != ''}
{html_options options=$fields.status_detail.options selected=$fields.status_detail.value}
{else}
{html_options options=$fields.status_detail.options selected=$fields.status_detail.default}
{/if}
</select>
{else}
{assign var="field_options" value=$fields.status_detail.options }
{capture name="field_val"}{$fields.status_detail.value}{/capture}
{assign var="field_val" value=$smarty.capture.field_val}
{capture name="ac_key"}{$fields.status_detail.name}{/capture}
{assign var="ac_key" value=$smarty.capture.ac_key}
<select style='display:none' name="{$fields.status_detail.name}" 
id="{$fields.status_detail.name}" 
title=''          
>
{if isset($fields.status_detail.value) && $fields.status_detail.value != ''}
...
<input
id="{$fields.status_detail.name}-input"
name="{$fields.status_detail.name}-input"
size="30"
value="{$field_val|lookup:$field_options}"
type="text" style="vertical-align: top;">
<span class="id-ff multiple">
<button type="button"><img src="{sugar_getimagepath file="id-ff-down.png"}" id="{$fields.status_detail.name}-image"></button><button type="button"
id="btn-clear-{$fields.status_detail.name}-input"
title="Clear"
onclick="SUGAR.clearRelateField(this.form, '{$fields.status_detail.name}-input', '{$fields.status_detail.name}');sync_{$fields.status_detail.name}()"><img src="{sugar_getimagepath file="id-ff-clear.png"}"></button>
</span>
{literal}
<script>
	SUGAR.AutoComplete.{/literal}{$ac_key}{literal} = [];
	{/literal}
			...
			{literal}
		(function (){
			var selectElem = document.getElementById("{/literal}{$fields.status_detail.name}{literal}");
	...
	SUGAR.AutoComplete.{$ac_key}.inputNode = Y.one('#{$fields.status_detail.name}-input');
	SUGAR.AutoComplete.{$ac_key}.inputImage = Y.one('#{$fields.status_detail.name}-image');
	SUGAR.AutoComplete.{$ac_key}.inputHidden = Y.one('#{$fields.status_detail.name}');
			...
			//global variable 
			sync_{/literal}{$fields.status_detail.name}{literal} = function(){
				SyncToHidden();
			}
			function syncFromHiddenToWidget(){
				...
				for (i=0;i<selectElem.options.length;i++){
					...
					if (selectElem.options[i].value==selectElem.value && document.activeElement != document.getElementById('{/literal}{$fields.status_detail.name}-input{literal}'))
						SUGAR.AutoComplete.{/literal}{$ac_key}{literal}.inputNode.set('value',selectElem.options[i].innerHTML);
				}
			}

            YAHOO.util.Event.onAvailable("{/literal}{$fields.status_detail.name}{literal}", syncFromHiddenToWidget);
		{/literal}
...
{capture name="label" assign="label"}{sugar_translate label='Note' module='te_disposition'}{/capture}
{$label|strip_semicolon}:
</td>
{counter name="fieldsUsed"}
...
{capture name="label" assign="label"}{sugar_translate label='LBL_DATEOFCALLBACK' module='te_disposition'}{/capture}
{$label|strip_semicolon}:
</td>
{counter name="fieldsUsed"}
...
{capture name="tr" assign="tableRow"}
<tr>
<td valign="top" id='te_disposition_leads_name_label' width='12.5%' scope="col">
{capture name="label" assign="label"}{sugar_translate label='LBL_TE_DISPOSITION_LEADS_FROM_LEADS_TITLE' module='te_disposition'}{/capture}
{$label|strip_semicolon}:
</td>
{counter name="fieldsUsed"}
...
<input type="text" name="{$fields.te_disposition_leads_name.name}" class="sqsEnabled" tabindex="0" id="{$fields.te_disposition_leads_name.name}" size="" value="{$fields.te_disposition_leads_name.value}" title='' autocomplete="off"  	 >
<input type="hidden" name="{$fields.te_disposition_leads_name.id_name}" 
id="{$fields.te_disposition_leads_name.id_name}" 
value="{$fields.te_disposition_leadsleads_ida.value}">
<span class="id-ff multiple">
<button type="button" name="btn_{$fields.te_disposition_leads_name.name}" id="btn_{$fields.te_disposition_leads_name.name}" tabindex="0" title="{sugar_translate label="LBL_SELECT_BUTTON_TITLE"}" class="button firstChild" value="{sugar_translate label="LBL_SELECT_BUTTON_LABEL"}"
onclick='open_popup(
"{$fields.te_disposition_leads_name.module}", 
600, 
400, 
"", 
true, 
false, 
{literal}{"call_back_function":"set_return","form_name":"EditView","field_to_name_array":{"id":"te_disposition_leadsleads_ida","name":"te_disposition_leads_name"}}{/literal}, 
"single", 
true
);' ><img src="{sugar_getimagepath file="id-ff-select.png"}"></button><button type="button" name="btn_clr_{$fields.te_disposition_leads_name.name}" id="btn_clr_{$fields.te_disposition_leads_name.name}" tabindex="0" title="{sugar_translate label="LBL_ACCESSKEY_CLEAR_RELATE_TITLE"}"  class="button lastChild"
onclick="SUGAR.clearRelateField(this.form, '{$fields.te_disposition_leads_name.name}', '{$fields.te_disposition_leads_name.id_name}');"  value="{sugar_translate label="LBL_ACCESSKEY_CLEAR_RELATE_LABEL"}" ><img src="{sugar_getimagepath file="id-ff-clear.png"}"></button>
</span>
<script type="text/javascript">
SUGAR.util.doWhen(
		"typeof(sqs_objects) != 'undefined' && typeof(sqs_objects['{$form_name}_{$fields.te_disposition_leads_name.name}']) != 'undefined'",
		enableQS
);
</script>
<td valign="top" id='date_of_followup_label' width='12.5%' scope="col">
{capture name="label" assign="label"}{sugar_translate label='LBL_DATEOFFOLLOWUP' module='te_disposition'}{/capture}
{$label|strip_semicolon}:
</td>
{counter name="fieldsUsed"}
...
{capture name="label" assign="label"}{sugar_translate label='LBL_DATEOFPROSPECT' module='te_disposition'}{/capture}
{$label|strip_semicolon}:
</td>
{counter name="fieldsUsed"}
...
<script type="text/javascript">
addForm('EditView');addToValidate('EditView', 'name', 'name', true,'{/literal}{sugar_translate label='LBL_NAME' module='te_disposition' for_js=true}{literal}' );
addToValidate('EditView', 'date_entered_date', 'date', false,'Date Created' );
addToValidate('EditView', 'date_modified_date', 'date', false,'Date Modified' );
addToValidate('EditView', 'modified_user_id', 'assigned_user_name', false,'{/literal}{sugar_translate label='LBL_MODIFIED' module='te_disposition' for_js=true}{literal}' );
addToValidate('EditView', 'modified_by_name', 'relate', false,'{/literal}{sugar_translate label='LBL_MODIFIED_NAME' module='te_disposition' for_js=true}{literal}' );
addToValidate('EditView', 'created_by', 'assigned_user_name', false,'{/literal}{sugar_translate label='LBL_CREATED' module='te_disposition' for_js=true}{literal}' );
addToValidate('EditView', 'created_by_name', 'relate', false,'{/literal}{sugar_translate label='LBL_CREATED' module='te_disposition' for_js=true}{literal}' );
addToValidate('EditView', 'description', 'text', false,'{/literal}{sugar_translate label='LBL_DESCRIPTION' module='te_disposition' for_js=true}{literal}' );
addToValidate('EditView', 'deleted', 'bool', false,'{/literal}{sugar_translate label='LBL_DELETED' module='te_disposition' for_js=true}{literal}' );
addToValidate('EditView', 'assigned_user_id', 'relate', false,'{/literal}{sugar_translate label='LBL_ASSIGNED_TO_ID' module='te_disposition' for_js=true}{literal}' );
addToValidate('EditView', 'assigned_user_name', 'relate', false,'{/literal}{sugar_translate label='LBL_ASSIGNED_TO_NAME' module='te_disposition' for_js=true}{literal}' );
addToValidate('EditView', 'status', 'enum', true,'{/literal}{sugar_translate label='LBL_STATUS' module='te_disposition' for_js=true}{literal}' );
addToValidate('EditView', 'status_detail', 'enum', false,'{/literal}{sugar_translate label='LBL_STATUS_DETAIL' module='te_disposition' for_js=true}{literal}' );
addToValidate('EditView', 'lead_mobile', 'varchar', false,'{/literal}{sugar_translate label='LBL_LEAD_MOBILE' module='te_disposition' for_js=true}{literal}' );
addToValidate('EditView', 'lead_email', 'varchar', false,'{/literal}{sugar_translate label='LBL_LEAD_EMAIL' module='te_disposition' for_js=true}{literal}' );
addToValidate('EditView', 'show_button', 'varchar', false,'{/literal}{sugar_translate label='LBL_UPDATE_DISPOSITION' module='te_disposition' for_js=true}{literal}' );
addToValidate('EditView', 'batch_name', 'varchar', false,'{/literal}{sugar_translate label='LBL_BATCH' module='te_disposition' for_js=true}{literal}' );
addToValidate('EditView', 'program_name', 'varchar', false,'{/literal}{sugar_translate label='LBL_PROGRAM' module='te_disposition' for_js=true}{literal}' );
addToValidate('EditView', 'te_disposition_leads_name', 'relate', false,'{/literal}{sugar_translate label='LBL_TE_DISPOSITION_LEADS_FROM_LEADS_TITLE' module='te_disposition' for_js=true}{literal}' );
...
addToValidate('EditView', 'unique_call_id', 'varchar', false,'{/literal}{sugar_translate label='LBL_UNIQUECALLID' module='te_disposition' for_js=true}{literal}' );
addToValidateBinaryDependency('EditView', 'assigned_user_name', 'alpha', false,'{/literal}{sugar_translate label='ERR_SQS_NO_MATCH_FIELD' module='te_disposition' for_js=true}{literal}: {/literal}{sugar_translate label='LBL_ASSIGNED_TO' module='te_disposition' for_js=true}{literal}', 'assigned_user_id' );
addToValidateBinaryDependency('EditView', 'te_disposition_leads_name', 'alpha', false,'{/literal}{sugar_translate label='ERR_SQS_NO_MATCH_FIELD' module='te_disposition' for_js=true}{literal}: {/literal}{sugar_translate label='LBL_TE_DISPOSITION_LEADS_FROM_LEADS_TITLE' module='te_disposition' for_js=true}{literal}', 'te_disposition_leadsleads_ida' );
</script><script language="javascript">if(typeof sqs_objects == 'undefined'){var sqs_objects = new Array;}sqs_objects['EditView_assigned_user_name']={"form":"EditView","method":"get_user_array","field_list":["user_name","id"],"populate_list":["assigned_user_name","assigned_user_id"],"required_list":["assigned_user_id"],"conditions":[{"name":"user_name","op":"like_custom","end":"%","value":""}],"limit":"30","no_match_text":"No Match"};sqs_objects['EditView_te_disposition_leads_name']={"form":"EditView","method":"query","modules":["Leads"],"group":"or","field_list":["name","id"],"populate_list":["te_disposition_leads_name","te_disposition_leadsleads_ida"],"required_list":["parent_id"],"conditions":[{"name":"name","op":"like_custom","end":"%","value":""}],"order":"name","limit":"30","no_match_text":"No Match"};</script>{/literal}
