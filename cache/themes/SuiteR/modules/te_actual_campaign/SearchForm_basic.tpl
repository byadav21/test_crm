...
<tr>
	...
	<td scope="row" nowrap="nowrap" width='1%' >
		...
		<label for='batch_basic' >{sugar_translate label='LBL_BATCH' module='te_actual_campaign'}</label>
    	</td>
...
<button type="button" name="btn_{$fields.batch_basic.name}"   title="{$APP.LBL_SELECT_BUTTON_TITLE}" class="button firstChild" value="{$APP.LBL_SELECT_BUTTON_LABEL}" onclick='open_popup("{$fields.batch_basic.module}", 600, 400, "", true, false, {literal}{"call_back_function":"set_return","form_name":"search_form","field_to_name_array":{"id":"te_ba_batch_id_c_basic","name":"batch_basic"}}{/literal}, "single", true);'>{sugar_getimage alt=$app_strings.LBL_ID_FF_SELECT name="id-ff-select" ext=".png" other_attributes=''}</button><button type="button" name="btn_clr_{$fields.batch_basic.name}"   title="{$APP.LBL_CLEAR_BUTTON_TITLE}" class="button lastChild" onclick="this.form.{$fields.batch_basic.name}.value = ''; this.form.{$fields.te_ba_batch_id_c_basic.name}.value = '';" value="{$APP.LBL_CLEAR_BUTTON_LABEL}">{sugar_getimage name="id-ff-clear" alt=$app_strings.LBL_ID_FF_CLEAR ext=".png" other_attributes=''}</button>
	...
	<td scope="row" nowrap="nowrap" width='1%' >
		...
		<label for='plan_date_basic' >{sugar_translate label='LBL_PLAN_DATE' module='te_actual_campaign'}</label>
    	</td>

	
	<td  nowrap="nowrap" width='1%'>
			
{assign var="id" value=$fields.plan_date_basic.name }

{if isset($smarty.request.plan_date_basic_range_choice)}
{assign var="starting_choice" value=$smarty.request.plan_date_basic_range_choice}
{else}
...
{html_options options=$fields.plan_date_basic.options selected=$starting_choice}
</select>
</div>

<div id="{$id}_range_div" style="{if preg_match('/^\[/', $smarty.request.range_plan_date_basic)  || $starting_choice == 'between'}display:none{else}display:''{/if};">
<input autocomplete="off" type="text" name="range_{$id}" id="range_{$id}" value='{if empty($smarty.request.range_plan_date_basic) && !empty($smarty.request.plan_date_basic)}{$smarty.request.plan_date_basic}{else}{$smarty.request.range_plan_date_basic}{/if}' title=''   size="11" style=" float:left;">
...
{assign var=date_value value=$fields.plan_date_basic.value }
<input autocomplete="off" type="text" name="start_range_{$id}" id="start_range_{$id}" value='{$smarty.request.start_range_plan_date_basic }' title=''  tabindex='' size="11" style="width:100px !important;">
...
{$APP.LBL_AND}
{assign var=date_value value=$fields.plan_date_basic.value }
<input autocomplete="off" type="text" name="end_range_{$id}" id="end_range_{$id}" value='{$smarty.request.end_range_plan_date_basic }' title=''  tabindex='' size="11" style="width:100px !important;" maxlength="10">
