...
echo $query = "SELECT leads.id AS lead_id,leads.date_entered,leads.date_modified,
				...
				leads.status_description,leads.campaign_id,leads.gender, leads.update_flag,
				...
				leads.assigned_date,leads.channel,leads.`dispositionName`,te_ba_batch.no_of_installments instalment_no,sum(te_payment_details.amount) as totalamount
			FROM leads 
...
while (($row = $db->fetchByAssoc($result)) != null) {
	...
	if($resultselect->num_rows==0 && isset($row['lead_id'])){
		$row['lead_converted']=$row['status']=='Converted'?'1':'0';
		$insertdata="INSERT INTO `dashboard_leads` ( `lead_id`, `lead_entered_date`, `lead_modified_date`, `modified_user_id`,`modified_user_name`,`assigned_user_id`, `assigned_user_name`,`reports_to_id`,`manager_name`, `lead_deleted`, `lead_converted`, `lead_converted_date`, `lead_source`, `vendor`, `lead_source_description`, `lead_status`, `lead_status_description`, `campaign_id`, `gender`, `no_attempts`, `amount`, `utm`, `fee_usd`, `fee_inr`, `utm_campaign`, `institute_name`, `institute_id`, `program_name`, `program_id`, `batch_name`, `batch_code`, `batch_status`, `lead_assigned_date`, `channel`, `disposition_name`, `no_instalment`, `neoxstatus`, `manager_id`, `batch_size`,`update_flag`,`totalamount`) VALUES (   '".$row['lead_id']."', '".$row['date_entered']."', '".$row['date_modified']."', '".$row['modified_user_id']."', '".$row['modified_user_name']."','".$row['assigned_user_id']."','".$row['assigned_user_name']."','".$row['reports_to_id']."','".$row['manager_name']."', '".$row['lead_deleted']."', '".$row['lead_converted']."', '".$row['lead_converted_date']."', '".$row['lead_source']."', '".$row['vendor']."', '".$row['lead_source_description']."', '".$row['status']."', '".$row['status_description']."', '".$row['campaign_id']."', '".$row['gender']."', '".$row['no_attempts']."', '".$row['amount']."', '".$row['utm']."', '".$row['fee_usd']."', '".$row['fee_inr']."', '".$row['utm_campaign']."', '".$row['institute_name']."', '".$row['institute_id']."', '".$row['program_name']."', '".$row['program_id']."', '".$row['batch_name']."', '".$row['batch_code']."', '".$row['batch_status']."', '".$row['lead_assigned_date']."', '".$row['channel']."', '".$row['disposition_name']."', '".$row['no_instalment']."', '".$row['neoxstatus']."', '".$row['manager_id']."', '".$row['batch_size']."','".$row['update_flag']."','".$row['totalamount']."');";
			$insertquerydata=$db->query($insertdata);
	}else{
		$row['lead_converted']=$row['status']=='Converted'?'1':'0';
		$updatedata="UPDATE `dashboard_leads` SET `lead_entered_date` = '".$row['date_entered']."', `lead_modified_date` = '".$row['date_modified']."', `modified_user_id` = '".$row['modified_user_id']."', `modified_user_name` = '".$row['modified_user_name']."', `assigned_user_id` = '".$row['assigned_user_id']."', `assigned_user_name` = '".$row['assigned_user_name']."', `reports_to_id` = '".$row['reports_to_id']."', `manager_name` = '".$row['manager_name']."', `lead_deleted` = '".$row['lead_deleted']."', `lead_converted` = '".$row['lead_converted']."', `lead_converted_date` = ".$row['lead_converted_date']."', `lead_source` = '".$row['lead_source']."', `vendor` = '".$row['vendor']."', `lead_source_description` = '".$row['lead_source_description']."', `lead_status` = '".$row['status']."', `lead_status_description` = '".$row['status_description']."', `campaign_id` = '".$row['campaign_id']."', `gender` = '".$row['gender']."', `no_attempts` = '".$row['no_attempts']."', `amount` = '".$row['amount']."', `utm` = '".$row['utm']."', `fee_usd` = '".$row['fee_usd']."', `fee_inr` = '".$row['fee_inr']."', `utm_campaign` = '".$row['utm_campaign']."', `institute_name` = '".$row['institute_name']."', `institute_id` =  '".$row['institute_id']."', `program_name` = '".$row['program_name']."', `program_id` = '".$row['program_id']."', `batch_name` = '".$row['batch_name']."', `batch_code` = '".$row['batch_code']."', `batch_status` = '".$row['batch_status']."', `channel` = '".$row['channel']."', `disposition_name` = '".$row['disposition_name']."', `no_instalment` = '".$row['no_instalment']."', `neoxstatus` = '".$row['neoxstatus']."', `manager_id` = '".$row['manager_id']."', `batch_size` = '".$row['batch_size']."', `update_flag` = '".$row['update_flag']."' WHERE `dashboard_leads`.`lead_id` = '".$row['lead_id']."';";
