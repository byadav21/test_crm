...
class syncsaptables_insert
{
    ...
    function StudCustomers()
    {
        ...
        $currentTime      = date('Y-m-d H:i:s');
         ...
         $query = "SELECT
                 ...
                 JOIN `te_student_payment` `sp` on `sb`.`id`=`sp`.`te_student_batch_id_c`
        ...
        $currentTime      = date('Y-m-d H:i:s');
         ...
         $query   = "SELECT 
                 ...
                 FROM (((((`te_student` `s`
                       ...
                       JOIN leads_te_payment_details_1_c AS lp ON lp.leads_te_payment_details_1te_payment_details_idb=pd.id
                       JOIN `leads` on((`sb`.`leads_id` = `leads`.`id`)))
                 WHERE pd.deleted=0   AND lp.deleted=0  AND `pd`.SAP_Status=5";
        $leadObj = mysqli_query($conn, $query);
        if ($leadObj)
        {
 ...
 function main_insert($startdate,$enddate)
    {
        ...
        $CustomersArr = $this->Customers($startdate,$enddate);
...
$mainObj = new syncsaptables_insert($startdate,$enddate);
$mainObj->main_insert();
